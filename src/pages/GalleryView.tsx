...
import { ChevronLeft, ChevronRight, Download, Package, X } from 'lucide-react'
...
export default function GalleryView() {
  ...
  async function handleDownloadOne(index: number) {
    ...
    try {
      ...
      setError('Não foi possível descarregar essa foto. Voltem a entrar e tentem de novo.')
    }
  }
  ...
  return (
    <div className="pt-28 sm:pt-32 pb-20 sm:pb-28 min-h-screen">
      ...
      <section className="container-px mb-10 sm:mb-14">
        <span className="label-sm">Galeria privada</span>
        <h1 className="mt-4 leading-[1.05]" style={{ fontSize: 'clamp(2rem, 5.5vw, 4rem)' }}>
          {gallery.title}
        </h1>
        {gallery.message && (
          <p className="text-sm text-titanium/55 leading-relaxed mt-5 max-w-lg">{gallery.message}</p>
        )}
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          {gallery.downloadEnabled && photos.length > 0 && (
            ...
            >
              <Package size={14} />
              {zipping
                ? `${zipping.phase} ${zipping.done}/${zipping.total}`
                : 'Descarregar tudo'}
            </button>
          )}
          {zipping && (
      ...
      <section className="container-px">
        {photos.length === 0 ? (
          <p className="text-sm text-titanium/45">
            Ainda não há fotografias nesta galeria. Avisamos assim que estiverem prontas.
          </p>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
            {photos.map((photo, i) => (
              <motion.button
                ...
                onClick={() => setOpen(i)}
                className="relative block overflow-hidden rounded-lg group aspect-[3/4] bg-white/[0.04]"
              ...
              >
                ...
                />
                <span className="absolute inset-0 bg-eerie/0 group-hover:bg-eerie/20 transition-colors" />
              </motion.button>
            ))}
          </div>
        )}
              ...
              <div className="flex items-center gap-1">
                {gallery.downloadEnabled && (
                  <button
                    ...
                    aria-label="Descarregar esta fotografia"
                  >
                    <Download size={20} />
                  </button>
                )}
